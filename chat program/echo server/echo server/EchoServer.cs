...
namespace echo_server
{
    public class EchoServer
    {
        ...
        {
            //소켓 생성- 다른호스트와 통신에 사용할 개체 생성
             Socket lissock = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp); // 네트워크 주소 체계, 소켓 타입 결정(전송 방식 선택), 프로토콜 선택
  ...
                DoIt(dosock);
        ...
        private void DoIt(Socket dosock) // 실제 연결한 클라이언트와 에코 서비스를 수행하는 DoIt 메서드를 구현
        {
            ...
            byte[] packet = new byte[1024]; // 수신할 버퍼 생성
            while (true)
            {
                //클라이언트로부터 데이터를 받음
